...
def try_rewire(tree, node, neighbors, obstacles, queue=None):
    ...
    for n in neighbors:
        ...
        if new_cost < n.cost:
            if line_collision_free(node.x, n.x, obstacles):
                # rewire
                if n.parent:
...
def random_rewire(tree, queue, obstacles):
    while queue:
        ...
        for near in neighbors:
            ...
            if c_new < c_old and line_collision_free(n_r.x, near.x, obstacles):
                if near.parent:
...
def root_rewire(tree, queue, obstacles,path,k=10):
    ...
    while queue and time.perf_counter() - start_time < REWIRE_BUDGET:
        ...
        for near in near_nodes:
            ...
            if c_new < c_old and line_collision_free(n_s.x, near.x, obstacles):
                if near.parent:
                    near.parent.children.remove(near)
                n_s.add_child(near)
                near.cost = c_new
