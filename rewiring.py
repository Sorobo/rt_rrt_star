...
def try_rewire(tree, node, neighbors, obstacles, rectangles=None, queue=None):
    ...
    for n in neighbors:
        ...
        if new_cost < n.cost:
            if boat_collision_free(node, n, obstacles, rectangles=rectangles):
                # rewire
                if n.parent:
...
def random_rewire(tree, queue, obstacles, rectangles=None):
    if rectangles is None:
        rectangles = []
    ...
    while queue:
        ...
        for near in neighbors:
            ...
            if c_new < c_old and boat_collision_free(n_r, near, obstacles, rectangles=rectangles):
                if near.parent:
...
def root_rewire(tree, queue, obstacles, path, rectangles=None, k=10):
    ...
    while queue and time.perf_counter() - start_time < REWIRE_BUDGET:
        ...
        for near in near_nodes:
            ...
            if c_new < c_old and boat_collision_free(n_s, near, obstacles, rectangles=rectangles):
                if near.parent:
                    near.parent.children.remove(near)
                n_s.add_child(near)
                near.cost = c_new
